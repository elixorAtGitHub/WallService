...
fun main() {
    ...
    //публикуем посты
    val post1 = Post(0,9,6, true,0,"ПОСТ #1 на стене пользователя 9 автора 6",17,3,true,false, emptyStringArray)
    WallService.add(post1)
    println(post1)
    val post2 = Post(0,6,4, false,0,"ПОСТ #2 на стене пользователя 6 автора 4",25,10,false,false, emptyStringArray)
    WallService.add(post2)
    println(post2)
    val post3 = Post(0,4,9, true,0,"ПОСТ #3 на стене пользователя 4 автора 9",68,18,false,false, emptyStringArray)
    ...
    val (id, ownerId, fromId, friendsOnly, date, text, views, likes, isPinned, isFavourite, attachments) = WallService.searchById(2)
    val post2Edited = Post(id, ownerId, fromId, friendsOnly, date, "ОБНОВЛЕННЫЙ " + text,views, likes, isPinned, isFavourite, attachments)
